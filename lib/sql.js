 ...
 */
SQLConnector.prototype.buildWhere = function(model, where, tableAlias, aliases) {
 ...
 * @private
 */
SQLConnector.prototype._buildWhere = function(model, where, tableAlias, aliases) {
  if (!where) {
  ...
  for (var key in where) {
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        let childWhere = self._buildWhere(relation.model, where[key], aliases[relation.model], aliases);
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        if (type == 'belongsTo') {
          ...
          joinThrough = `LEFT JOIN ${throughModel} ${throughModelAlias} ON ${throughModelAlias}.${parentForeignKey}` +
            ` = ${tableAlias}.id`;
