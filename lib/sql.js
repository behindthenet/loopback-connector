...
SQLConnector.prototype.exists = function(model, id, options, cb) {
  ...
  var where = {};
  where[idName] = id;
  var tableAlias = 't' + Math.random().toString().replace('.', '');
...
Connector.defineAliases(SQLConnector.prototype, 'destroy', ['delete', 'deleteById',
  'destroyById']);
 ...
 */
SQLConnector.prototype.buildWhere = function(model, where, tableAlias) {
  var whereClause = this._buildWhere(model, where, tableAlias);
  if (whereClause.sql) {
 ...
 * @private
 */
SQLConnector.prototype._buildWhere = function(model, where, tableAlias) {
  if (!where) {
  ...
  for (var key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          var stmtForClause = self._buildWhere(model, clauses[i], tableAlias);
    ...
    var columnName = tableAlias + '.' + self.columnEscaped(model, key);
 ...
 */
SQLConnector.prototype.buildOrderBy = function(model, order, aliases) {
  if (!order) {
    return '';
  }
  var self = this;
      ...
      if (key.indexOf('.') > -1) {
        ...
        if (relations && relations[modelAndProperty[0]]) {
          ...
          if (alias) {
            ...
            columnNames.push(alias + '.' + modelAndProperty[1] + ' as ' + alias + '_orderBy' + modelAndProperty[1]);
          }
        }
      } else {
 ...
 */
SQLConnector.prototype.buildColumnNames = function(model, filter, tableAlias) {
  ...
  var names = keys.map(function(c) {
    return tableAlias + '.' + self.columnEscaped(model, c);
  });
  return names.join(',');
};
  ...
  if (filter) {
    ...
    if (filter.where) {
      ...
      if (joins.joinStmt.sql) {
        ...
        selectStmt.sql = selectStmt.sql.replace('SELECT', 'SELECT DISTINCT ' + tableAlias + '.id, ');
        selectStmt.merge(joins.joinStmt);
      }
      var whereStmt = this.buildWhere(model, filter.where, tableAlias);
    ...
    if (filter.order) {
      ...
      if (order.columnNames) {
        selectStmt.sql = selectStmt.sql.replace('FROM', ', ' + order.columnNames +' FROM ');
      }
    }
...
SQLConnector.prototype.count = function(model, where, options, cb) {
  ...
  stmt = stmt.merge(this.buildWhere(model, where, tableAlias));
...
SQLConnector.prototype.buildJoins = function (model, where, tableAlias) { 
  var self = this;
  var props = self.getModelDefinition(model).properties;
  ...
  for (var key in where) {
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        let childTableAlias = 't' + Math.random().toString().replace('.', '');
        aliases[relation.model] = childTableAlias;
        let childWhere = self._buildWhere(relation.model, where[key], childTableAlias);
        ...
        if (type == 'belongsTo') {
          ...
          const throughModelAlias = 't' + Math.random().toString().replace('.', '');
          aliases[relation.through] = throughModelAlias;
          let parentForeignKey = model.toLowerCase() + 'Id';
          let childForeignKey = relation.model.toLowerCase() + 'Id';
          joinThrough = `INNER JOIN ${throughModel} ${throughModelAlias} ON ${throughModelAlias}.${parentForeignKey}` +
            ` = ${tableAlias}.id`;
          joinOn = throughModelAlias + '.' + childForeignKey + ' = ' + childTableAlias + '.id';
        } else {
        ...
        let join = new ParameterizedSQL(
          `INNER JOIN ${joinTable} ${childTableAlias} ON ${joinOn}`, []);
        if (joinThrough) {
          join = new ParameterizedSQL(joinThrough, []).merge(join);
        }
        if (childWhere.sql) {
          join.merge(childWhere.merge(') '), ' AND (');
        }
        var recursiveResult = self.buildJoins(relation.model, where[key], childTableAlias);
