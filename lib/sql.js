    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        if (type == 'belongsTo') {
          ...
          joinWhere = throughModelAlias + '.' + parentForeignKey +
            ' = ' + tableAlias + '.id';
          joinWhere += ' AND ' + throughModelAlias + '.' + childForeignKey +
            ' = ' + childTableAlias + '.id';
        } else {
          joinWhere = childTableAlias + '.' + foreignKey + ' = ' + tableAlias + '.id';
        }
        let subselect = new ParameterizedSQL(
          `SELECT COUNT(1) FROM ${relation.model} ${childTableAlias} 
          ${throughModel} ${throughModelAlias} WHERE ${joinWhere}`, []);
        subselect.merge(childWhere.merge(')'), ' AND ( ');
