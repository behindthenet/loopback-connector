...
Connector.defineAliases(SQLConnector.prototype, 'destroy', ['delete', 'deleteById', 'destroyById']);
  ...
  for (var key in where) {
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        let foreignKey = relation.foreignKey != '' ? relation.foreignKey : relation.model.toLowerCase() + 'Id';
        let type = relation.type;
        let throughModel = '';
        let throughModelAlias = '';
        let joinWhere = '';
        if (type == 'belongsTo') {
          joinWhere = tableAlias + '.' + foreignKey + ' = ' + childTableAlias + '.id';
        } else if (type == 'hasMany' && relation.through) {
          throughModel = ', ' + relation.through;
          throughModelAlias = 't' + Math.random().toString().replace('.', '');
          let parentForeignKey = model.toLowerCase() + 'Id';
          let childForeignKey = relation.model.toLowerCase() + 'Id';
          joinWhere = throughModelAlias + '.' + parentForeignKey + ' = ' + tableAlias + '.id';
          joinWhere += ' AND ' + throughModelAlias + '.' + childForeignKey + ' = ' + childTableAlias + '.id';
        } else {
          joinWhere = childTableAlias + '.' + foreignKey + ' = ' + tableAlias + '.id';
        }
        let subselect = new ParameterizedSQL(
          `SELECT COUNT(1) FROM ${relation.model} ${childTableAlias} ${throughModel} ${throughModelAlias}
          WHERE ${joinWhere}`, []);
        subselect.merge(childWhere, ' AND');
        let subselectCount = new ParameterizedSQL({
          sql: ' 0 < ( ' + subselect.sql + ' )',
          params: subselect.params
        });
        whereStmts.push(subselectCount);
      } else {
